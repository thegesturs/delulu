...
): Effect.Effect<TwitterProfile, IntegrationError, ConvexClient> =>
  Effect.gen(function* () {
...
const downloadFile = (
  fileUrl: string
): Effect.Effect<{ buffer: Buffer; mimeType: string }, IntegrationError> =>
  Effect.tryPromise({
    try: () =>
...
const finalizeUpload = (
  client: Client,
  mediaId: string
): Effect.Effect<{ processingRequired: boolean }, IntegrationError> =>
  Effect.tryPromise({
...
): Effect.Effect<Tweet[], IntegrationError> => {
  if (!content.length) {
...
): Effect.Effect<{ tweetId: string }, IntegrationError> =>
  Effect.gen(function* () {
