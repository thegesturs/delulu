...
): Effect.Effect<BlueskyProfile, IntegrationError, ConvexClient> =>
  Effect.gen(function* () {
...
const uploadMediaBlob = (
  mediaUrl: string,
  accessToken: string
): Effect.Effect<BlueskyBlobResponse, IntegrationError> =>
  Effect.gen(function* () {
...
): Effect.Effect<BlueskyPostResponse, IntegrationError> =>
  Effect.gen(function* () {
