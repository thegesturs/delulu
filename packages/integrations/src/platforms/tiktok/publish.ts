...
): Effect.Effect<TikTokProfile, IntegrationError, ConvexClient> =>
  Effect.gen(function* () {
...
const checkPostStatus = (
  accessToken: string,
  publishId: string
): Effect.Effect<{ status: string; fail_reason?: string }, IntegrationError> =>
  Effect.gen(function* () {
    const data = yield* Effect.tryPromise({
      try: () =>
        axios
...
const getRecentVideoId = (
  ...
  attempt = 0,
  maxRetries = 3
): Effect.Effect<string | null, IntegrationError> =>
  Effect.gen(function* () {
