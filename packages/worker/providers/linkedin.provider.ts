  ...
  return ResultAsync.fromPromise(
    ...
    return validateVideoForLinkedIn(videoBuffer, videoUrl).andThen(() => {
      ...
      return ResultAsync.fromPromise(
        ...
        console.log(
          `[LinkedIn Video] Upload initialized - Video URN: ${assetUrn}, Upload URLs: ${uploadInstructions.length}`
        );
        ...
        console.log(
          `[LinkedIn Video] Splitting into ${chunks.length} chunks of ~${(chunkSize / 1024 / 1024).toFixed(2)}MB each`
        );
        ...
        return ResultAsync.combine(uploadChunks).andThen((uploadedPartIds) => {
          ...
          return ResultAsync.fromPromise(
            ...
            console.log(
              `[LinkedIn Video] Upload finalized successfully: ${assetUrn}`
            );
            return assetUrn;
          });
        });
