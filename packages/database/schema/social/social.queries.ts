...
export const socialQueries = {
  ...
  getSocialProviderById: (id: string) => {
    return database.select().from(socialProviders).where(eq(socialProviders.id, id)).limit(1);
  },

  getUserSocialProviders: (userId: string) => {
    return database.select().from(socialProviders)
      .where(and(eq(socialProviders.userId, userId), eq(socialProviders.isActive, true)))
      .orderBy(desc(socialProviders.createdAt));
  },

  getOrganizationSocialProviders: (organizationId: string) => {
    return database.select().from(socialProviders)
      .where(and(eq(socialProviders.organizationId, organizationId), eq(socialProviders.isActive, true)))
      .orderBy(desc(socialProviders.createdAt));
  },

  getSocialProviderByType: (userId: string, socialType: string) => {
    return database.select().from(socialProviders)
      .where(and(
        eq(socialProviders.userId, userId),
        eq(socialProviders.socialType, socialType as any),
  ...
  updateSocialProvider: (id: string, providerData: Partial<typeof socialProviders.$inferInsert>) => {
    return database.update(socialProviders).set(providerData).where(eq(socialProviders.id, id)).returning();
  },

  deactivateSocialProvider: (id: string) => {
    return database.update(socialProviders).set({ isActive: false }).where(eq(socialProviders.id, id)).returning();
  },

  reactivateSocialProvider: (id: string) => {
    return database.update(socialProviders).set({ isActive: true }).where(eq(socialProviders.id, id)).returning();
  },
  ...
  updateSocialProviderSync: (id: string) => {
    return database.update(socialProviders)
      .set({ lastSyncedAt: new Date() })
      .where(eq(socialProviders.id, id))
      .returning();
  },

  getExpiredTokens: (beforeDate: Date = new Date()) => {
    return database.select().from(socialProviders)
      .where(and(
        eq(socialProviders.isActive, true),
